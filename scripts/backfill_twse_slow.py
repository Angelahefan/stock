#!/usr/bin/env python3
"""
backfill_twse_slow.py — TWSE 5yr price backfill, 1 ticker at a time.
30s sleep between tickers. Skips tickers that already have data.
"""
import sys
import time
import random
...
logger = setup_logging("backfill_twse_slow")


def main():
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT ticker, yf_symbol FROM datapai.ticker_universe WHERE exchange = %s AND is_active", ["TWSE"])
            tickers = cur.fetchall()
            cur.execute("SELECT DISTINCT ticker FROM datapai.prices WHERE exchange = %s", ["TWSE"])
            done = set(r[0] for r in cur.fetchall())

    todo = [(t, y) for t, y in tickers if t not in done]
    logger.info("TWSE: %d total, %d done, %d remaining", len(tickers), len(done), len(todo))
        ...
        try:
            ...
            if df is not None and not df.empty:
                ...
                for ts, row in df.iterrows():
                    ...
                    rows.append((db_t, td, o, h, l, cv, cv, v, "TWSE", "yfinance"))
                if rows:
                    upsert_daily_rows(rows, batch_label=f"TWSE {db_t}")
                    total += len(rows)
